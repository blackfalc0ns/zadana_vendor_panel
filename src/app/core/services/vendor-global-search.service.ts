...
export class VendorGlobalSearchService {
 ...
 titleAr: 'لوحة التحكم',
 titleEn: 'Dashboard',
 subtitleAr: 'لمحة عامة على أداء المتجر والعمليات المفتوحة',
 ...
 entry.titleEn,
 entry.subtitleAr,
 entry.subtitleEn,...entry.keywords
 ...
 }).filter((item) => item.score > 0).sort((a, b) => b.score - a.score).slice(0, this.maxResultsPerSource).map(({ entry }) => ({
 id: entry.id,
 type: 'navigation',
 ...
 return parts.map((part) => (part ?? '').trim()).filter((part) => part.length > 0 && part!== '-').join(' • ');
 }

 private normalizeText(value: string): string {
 return value.toLocaleLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[أإآ]/g, 'ا').replace(/ى/g, 'ي').replace(/ة/g, 'ه').replace(/[^\p{L}\p{N}\s]/gu, ' ').replace(/\s+/g, ' ').trim();
 }
}
