...
export class HeaderComponent implements OnInit {
 ...
 this.vendorAuthService.currentUser$.pipe(takeUntilDestroyed(this.destroyRef)).subscribe((user) => {
 this.syncWorkspaceContext(user);
 this.cdr.markForCheck();
 });

 this.profileService.getProfile().pipe(takeUntilDestroyed(this.destroyRef)).subscribe((profile) => {
