...
@Component({
 ...
 `,
 styles: [`
 :host { display: block; }.no-scrollbar::-webkit-scrollbar { display: none; }.no-scrollbar { -ms-overflow-style: none; scrollbar-width: none; }
 `]
})
export class ProductListComponent implements OnInit, OnDestroy {
 ...
 return [
 { value: '', labelKey: 'PRODUCTS.FILTERS.ALL_CATEGORIES' },...this.availableCategories.map(c => ({ value: c.value, label: c.label }))
 ];
 }
 ...
 return [
 { value: '', labelKey: 'PRODUCTS.FILTERS.ALL_PACKAGE_TYPES' },...this.units.filter((unit) => unit.kind === 'Packaging').map((unit) => ({
 value: unit.id,
 ...
 return [
 { value: '', labelKey: 'PRODUCTS.FILTERS.ALL_MEASUREMENT_UNITS' },...this.units.filter((unit) => unit.kind === 'Measurement').map((unit) => ({
 value: unit.id,
 label: this.currentLang === 'ar' ? (unit.nameAr || unit.nameEn) : (unit.nameEn || unit.nameAr)
 }))
 ];
 ...
 this.products.map((product) => product.measurementValue).filter((value): value is number => typeof value === 'number')
 )).sort((left, right) => left - right);

 return [
 { value: '', labelKey: 'PRODUCTS.FILTERS.ALL_SIZES' },...values.map((value) => ({
 value: String(value),
 label: String(value)
 }))
 ];
 ...
 category,...this.flattenCategories(category.subCategories || [])
 ]);
 }
 ...
 this.alertModalService.success(
      this.currentLang === 'ar' ? 'حذفنا المنتج بنجاح.' : 'Product deleted successfully.'
 );
 this.loadProducts();
 },
 error: (error) => {
 ...
 error?.error?.message
 || (this.currentLang === 'ar'
 ? 'ما قدرنا نحذف المنتج الحين. تأكد أنه غير مرتبط بطلبات أو حملات.'
