...
export class BranchDetailComponent implements OnInit, OnDestroy {
 ...
 this.isEditingProfile = false;
 this.showFeedback(this.currentLang === 'ar' ? 'حدّثنا بروفايل الفرع.' : 'Branch profile updated.', 'success');
 this.cdr.markForCheck();
 },
 error: (error) => {
 this.isSavingProfile = false;
 this.showFeedback(error?.error?.message || (this.currentLang === 'ar' ? 'ما قدرنا حفظ بروفايل الفرع.' : 'Unable to save branch profile.'), 'info');
 this.cdr.markForCheck();
 }
 });
 }
 ...
 next: () => {
 this.showFeedback(this.currentLang === 'ar' ? 'حدّثنا حالة الفرع.' : 'Branch status updated.', 'success');
 this.cdr.markForCheck();
 },
 error: () => {
 this.showFeedback(this.currentLang === 'ar' ? 'ما قدرنا نحدّث حالة الفرع.' : 'Unable to update branch status.', 'info');
 this.cdr.markForCheck();
 }
 });
 }
