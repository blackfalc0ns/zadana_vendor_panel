...
export class SupportTicketDetailComponent implements OnInit, OnDestroy {
 ...
 this.ticket = ticket;
 this.relatedArticles = articles.filter((article) => article.category === ticket.category).sort((first, second) => new Date(second.updatedAt).getTime() - new Date(first.updatedAt).getTime());
 });
 }
