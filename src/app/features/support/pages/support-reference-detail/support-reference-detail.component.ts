...
export class SupportReferenceDetailComponent implements OnInit, OnDestroy {
 ...
 this.relatedTickets = tickets.filter((ticket) => ticket.category === article.category).sort((first, second) => new Date(second.updatedAt).getTime() - new Date(first.updatedAt).getTime());
 });
 }
