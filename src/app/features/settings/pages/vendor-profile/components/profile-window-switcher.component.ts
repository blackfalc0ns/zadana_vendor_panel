      ...
      <button
        *ngFor="let window of windows; let first = first; let last = last"
        ...
        [ngClass]="{
          ...
          'sm:rounded-l-2xl': (first && currentLang !== 'ar') || (last && currentLang === 'ar'),
          'sm:rounded-r-2xl': (last && currentLang !== 'ar') || (first && currentLang === 'ar')
        }">
        <div class="profile-window-switcher__tab-meta">
          ...
          <span class="rounded-full px-2 py-0.5 text-[0.62rem] font-black transition-colors sm:hidden"
        ...
        <span class="hidden rounded-full px-2.5 py-0.5 text-[0.65rem] font-black transition-colors sm:inline-flex"
          [ngClass]="activeWindowId === window.id ? 'bg-gradient-to-r from-zadna-primary/20 to-teal-500/20 text-teal-800' : 'bg-slate-200/60 text-slate-600'">
          {{ counts[window.id] ?? 0 }}
        </span>

        <span aria-hidden="true" class="absolute inset-x-2 bottom-0 hidden h-1 rounded-full transition-all duration-300 sm:inset-x-0 sm:block sm:rounded-none"
