...
@Component({
  ...
  template: `
    <nav class="flex overflow-x-auto bg-white/40 backdrop-blur-xl" aria-label="Tabs">
      <button
        *ngFor="let window of windows; let first = first; let last = last"
        type="button"
        (click)="windowChange.emit(window.id)"
        class="group relative min-w-0 flex-1 overflow-hidden py-4 px-4 text-center text-[0.85rem] font-bold transition-all hover:bg-white/60 focus:z-10 focus:outline-none"
        [ngClass]="{
          'text-zadna-primary bg-white/50': activeWindowId === window.id,
          'text-slate-500 hover:text-slate-700': activeWindowId !== window.id,
          'rounded-l-2xl': (first && currentLang !== 'ar') || (last && currentLang === 'ar'),
          'rounded-r-2xl': (last && currentLang !== 'ar') || (first && currentLang === 'ar')
        }">
        <div class="flex items-center justify-center gap-2">
          ...
          <span class="ml-1 rounded-full px-2.5 py-0.5 text-[0.65rem] font-black transition-colors"
        ...
        <span aria-hidden="true" class="absolute inset-x-0 bottom-0 h-1 transition-all duration-300"
