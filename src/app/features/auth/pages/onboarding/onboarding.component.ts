 ...
 */
 ...
 ? 'لازم ترفع السجل التجاري بصيغة PDF.'
 ...
 if (!this.isPdfFile(file)) {
 this.submissionError = this.isRTL
 ? 'لازم ترفع الشهادة الضريبية بصيغة PDF.'
 ...
 */
 ...
 this.allCities = this.allCities.map((city) => ({...city,
 label: this.localizeName(city.nameAr, city.nameEn, lang)
 }));

 this.filteredCities = this.filteredCities.map((city) => ({...city,
 label: this.localizeName(city.nameAr, city.nameEn, lang)
 }));
 }
 ...
 return [this.storeLogo, this.crDocument, this.taxDocument, this.licenseDocument].filter(Boolean).length;
 }
 ...
 const total = Array.from(this.uploadProgressByKey.values()).reduce((sum, value) => sum + value, 0);
 ...
 const items = (profile.reviewItems || []).filter((item) => item.status === 'changes_requested').map((item) => [item.code, item] as [string, VendorReviewItem]);
 ...
 const rejectedSteps = Array.from(this.rejectedReviewItems.values()).filter((item) => typeof item.step === 'number').map((item) => item.step as number).sort((left, right) => left - right);
