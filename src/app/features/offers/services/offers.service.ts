...
export class OffersService {
 ...
 id: `campaign-${payload.categoryId}-${Date.now()}`,...payload
 };
 ...
 id: `clearance-${payload.productId}`,...payload
 };
 ...
 return products.filter((product) => product.stockQty > 0 && product.stockQty <= 12).sort((first, second) => first.stockQty - second.stockQty).slice(0, 6).map((product) => {
