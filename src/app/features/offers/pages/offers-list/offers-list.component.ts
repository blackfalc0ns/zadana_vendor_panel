...
export class OffersListComponent implements OnInit, OnDestroy {
 ...
 return [
 { value: '', labelKey: 'OFFERS.FILTERS.ALL_CATEGORIES' },...this.availableDirectCategories
 ];
 }

 get campaignCatOptions(): SearchableSelectOption[] {
 return [
 { value: '', labelKey: 'OFFERS.FILTERS.ALL_CATEGORIES' },...this.availableCampaignCategories
 ];
 }

 get clearanceCatOptions(): SearchableSelectOption[] {
 return [
 { value: '', labelKey: 'OFFERS.FILTERS.ALL_CATEGORIES' },...this.availableClearanceCategories
 ];
 }
 ...
 return this.vendorProducts.filter((product) => product.stockQty > 0 && product.stockQty <= 20).sort((first, second) => first.stockQty - second.stockQty);
 }
 ...
 ? 'استغرقت عملية حفظ الكوبون وقتًا أطول من المتوقع. جرّب مرة ثانية.'
 ...
 if (error.status === 403) {
 return this.currentLang === 'ar'
 ? 'ما عندك صلاحية لإنشاء أو تعديل الكوبونات لهذا الحساب.'
 ...
 return this.currentLang === 'ar'
 ? 'ما قدرنا حفظ الكوبون الحين. جرّب مرة ثانية.'
