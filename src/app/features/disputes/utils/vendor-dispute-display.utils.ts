...
export function getVendorDisputeDisplayTypeMetaLabel(dispute: VendorDisputeLike, language: string): string {
  ...
  switch (settlementStatus) {
    ...
    case 'rejected':
      return isArabic ? 'تم رفض طلب الإرجاع' : 'The return request was rejected';
  }
  ...
  if (status === 'approved') {
    ...
    return isArabic ? 'تمت الموافقة ويجري استكمال الإجراء' : 'Approved and awaiting final completion';
  }
