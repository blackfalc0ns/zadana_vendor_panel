...
@Component({
 ...
 styles: [`
 :host { display: block; }.line-clamp-2 {
 display: -webkit-box;
 -webkit-line-clamp: 2;
...
export class AlertsCenterPageComponent implements OnInit, DoCheck, OnDestroy {
 ...
 return this.alerts.filter((alert) => this.matchesQuickView(alert)).filter((alert) => this.matchesFilters(alert));
 }
 ...
 return (['today', 'yesterday', 'earlier'] as AlertDateGroupKey[]).filter((key) => groups.has(key)).map((key) => ({
 key,
 ...
 if (diffMs < minute) {
 return this.currentLang === 'ar' ? 'الحين' : 'Just now';
 }
