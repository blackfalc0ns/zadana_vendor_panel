...
export class VendorDashboardComponent implements OnInit, OnDestroy {
 ...
 this.staffBranchesService.getBranches().pipe(takeUntil(this.destroy$)).subscribe((branches) => {
 ...
 timer(DASHBOARD_REFRESH_MS, DASHBOARD_REFRESH_MS).pipe(
 ...
 const monthIndex = VendorDashboardComponent.EN_MONTH_ABBREVS.findIndex((month) => month.toLowerCase() === label.toLowerCase());
 if (monthIndex >= 0) {
 const monthDate = new Date(Date.UTC(2026, monthIndex, 1));
 return new Intl.DateTimeFormat(this.currentLang === 'ar' ? 'ar-SA' : 'en-US', { timeZone: 'Asia/Riyadh', month: 'short' }).format(monthDate);
 }

 const weekdayIndex = VendorDashboardComponent.EN_WEEKDAY_ABBREVS.findIndex((weekday) => weekday.toLowerCase() === label.toLowerCase());
 if (weekdayIndex >= 0) {
 const weekdayDate = new Date(Date.UTC(2026, 0, 4 + weekdayIndex));
 return new Intl.DateTimeFormat(this.currentLang === 'ar' ? 'ar-SA' : 'en-US', { timeZone: 'Asia/Riyadh', weekday: 'short' }).format(weekdayDate);
 }
 ...
 return new Intl.DateTimeFormat(this.currentLang === 'ar' ? 'ar-SA' : 'en-US', { timeZone: 'Asia/Riyadh', weekday: 'short' }).format(weekday);
 }
}
